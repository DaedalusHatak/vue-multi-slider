<template>
  <div :style="{ padding: padding }" class="carousel" :class="props.overflow ? '' : 'overflow'">
    <button
      v-if="wasTriggered && !isMobile"
      @click="back()"
      @mouseover="hoverButtons = true"
      @mouseleave="hoverButtons = false"
      :style="{ left: `${props.buttonPadding}rem`,width:`${props.buttonWidth}rem` }"
      :class="hoverButtons || props.buttonVisibile ? '' : 'button-visible'"
      class="handle left-handle"
      aria-label="Show previous movies"
    >
      <img class="hover-button" v-if="hoverButtons" src="../../public/chevron-left-solid.svg" alt="" />
    </button>
    ...
    <button
      ...
      @mouseleave="hoverButtons = false"
      @click="forward()"
      :style="{ right: `${props.buttonPadding}rem`,width:`${props.buttonWidth}rem` }"
      :class="hoverButtons || props.buttonVisibile ? '' : 'button-visible'"
      class="handle right-handle"
      aria-label="Show more movies"
    >
      <img src="../../public/chevron-right-solid.svg" alt="" />
    </button>
...
function touchEnd() {
  ...
  else if (touchMovement.value.start < touchMovement.value.end && touchMovement.value.end > 0)
    back()
}
...
function setVariable() {
  ...
  } else if (window.innerWidth >= 1280 && window.innerWidth < 1536 && props.xl) {
    screenVariable.value = props.xl
  } else if (window.innerWidth >= 1024 && window.innerWidth < 1280 && props.lg) {
